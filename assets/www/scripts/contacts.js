...
/** function called when the form for editing/creating contact has finished loading */
function onFormLoad() {
...
/** Called to display detailed information about a given contact retrieved from the address book */
...
function getParameterByName(name) {
	...
	var results = regex.exec(currentUlr);
	if(results == null)
		return "";
	else
...
/** Called to display contacts retrieved from the address book */
...
function populateForm(contacts) {
	if(contacts.length > 0) {
		var contact = contacts[0];
		var form = document.getElementsByTagName('from')[0].elements;
...
//$(document).bind("pageload", onContactsLoad);